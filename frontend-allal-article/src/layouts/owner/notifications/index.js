...
export default function OwnerNotifications() {
  ...
  const visible = useMemo(() => {
    if (category === "all") return mockOwnerNotifications;
    return mockOwnerNotifications.filter((item) => item.category === category);
  }, [category]);

  const stats = {
    unread: mockOwnerNotifications.filter((item) => !item.isRead).length,
    critical: mockOwnerNotifications.filter((item) => item.severity === "critical").length,
    action: mockOwnerNotifications.filter((item) => item.severity === "action_required").length,
    escalated: mockOwnerNotifications.filter((item) => item.state === "escalated").length,
  };

  const categories = [...new Set(mockOwnerNotifications.map((item) => item.category))];
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, flexWrap: "wrap", mb: 3 }}>
          <Box>
            ...
            <Box sx={{ fontSize: 13, color: "#8392ab" }}>
              اشتراكات، تهيئة مشتركين، حدود الخطط، وطلبات الربط بين المشتركين
            </Box>
          </Box>
          <Select size="small" value={category} onChange={(event) => setCategory(event.target.value)} sx={{ minWidth: 180, background: "#fff" }}>
                  ...
                  return (
                    <TableRow key={item.publicId} sx={{ "&:hover": { background: "#f8f9fa" } }}>
                      ...
                      <TableCell sx={{ minWidth: 260 }}>
                        ...
                        <Box sx={{ fontSize: 11, color: "#344767", mt: 0.5 }}>{item.reason}</Box>
                      </TableCell>
                      <TableCell>
