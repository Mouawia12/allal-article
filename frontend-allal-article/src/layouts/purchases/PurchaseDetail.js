...
const actionText = {
  confirm: {
    title: "تأكيد أمر الشراء",
    body: "سيتم تحويل الأمر إلى حالة مؤكد ليظهر كطلب جاهز للاستلام.",
    button: "تأكيد",
    color: "success",
  },
  receive: {
    title: "استلام البضاعة",
    body: "سيتم محاكاة استلام كل الكميات وتحديث حالة الأمر إلى مستلم.",
    button: "استلام",
    color: "info",
  },
  invoice: {
...
export default function PurchaseDetail() {
  ...
  const totals = useMemo(() => {
    ...
    return { untaxed, tax: total - untaxed, total, receivedQty, orderedQty };
  }, [receivedLines]);
  ...
  const runAction = () => {
    if (dialog === "confirm") {
      setStatus("confirmed");
      appendActivity("أكد أمر الشراء");
    }
    if (dialog === "receive") {
      setStatus("received");
      setReceivedBy("المستخدم الحالي");
      setReceivedLines((lines) => lines.map((line) => ({ ...line, receivedQty: line.qty })));
      appendActivity("سجل استلام كامل الكميات");
    }
    if (dialog === "invoice") {
    ...
    <DashboardLayout>
      ...
      <SoftBox py={3}>
        ...
        <Grid container spacing={2} mb={3}>
          ...
          <Grid item xs={12} sm={6} md={3}>
            <MetricCard label="الكميات المستلمة" value={`${totals.receivedQty} / ${totals.orderedQty}`} color="#82d616" icon={LocalShippingIcon} />
          </Grid>
        </Grid>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "#f8f9fa" }}>
                      {["الكود", "الصنف", "المطلوب", "المستلم", "الوحدة", "سعر الوحدة", "TVA", "الإجمالي"].map((header) => (
                  ...
                  <tbody>
                    {receivedLines.map((line, index) => (
                      <tr key={line.id} style={{ borderBottom: "1px solid #f0f2f5", background: index % 2 === 0 ? "#fff" : "#fafbfc" }}>
                        <td style={{ padding: "10px 12px" }}><SoftTypography variant="caption" color="info" fontWeight="bold">{line.productCode}</SoftTypography></td>
                        <td style={{ padding: "10px 12px" }}><SoftTypography variant="caption" fontWeight="medium">{line.product}</SoftTypography></td>
                        <td style={{ padding: "10px 12px" }}><SoftTypography variant="caption">{line.qty}</SoftTypography></td>
                        <td style={{ padding: "10px 12px" }}>
                          <SoftTypography variant="caption" fontWeight="bold" sx={{ color: line.receivedQty >= line.qty ? "#82d616" : "#fb8c00" }}>
                            {line.receivedQty}
                          </SoftTypography>
                        </td>
